...
class AmazonCreatorsAsyncClient:
    ...
    async def _request(self, operation: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 1. Wait for Rate Limiter Capacity (1 TPS)
        ...
        # V2.x: -H "Authorization: Bearer YOUR_ACCESS_TOKEN, Version YOUR_CREDENTIAL_VERSION"
        # V3.x: -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
        ...
        if response.status_code == 200:
            ...
            raise APIError(f"API Error ({response.status_code}): {response.text}", status_code=response.status_code)
